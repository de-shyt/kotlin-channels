...
class BufferedChannel<E>(private val capacity: Long) : Channel<E> {
    ...
    override suspend fun send(elem: E) {
        ...
        while (true) {
            ...
            when (updateCellOnSend(s, segment, index)) {
                ...
                RESULT_RENDEZVOUS -> {
                    // Both `send(e)` and `receive()` operations have reached the current segment,
                    // which means all previous segments were processed and should be unreachable.
                    // Clean the `prev` link of the segment to avoid memory leaks.
                    segment.cleanPrev()
                    return
                }
        ...
        while (true) {
            ...
            when (updateCellOnReceive(r, segment, index)) {
                ...
                RESULT_RENDEZVOUS -> {
                    ...
                    // Clean the `prev` link of the segment to avoid memory leaks.
                    segment.cleanPrev()
                    // Retrieve the element from the cell
                    return segment.retrieveElement(index)
                }
                // The cell was poisoned or stores an interrupted sender => restart
                RESULT_FAILED -> {
                    // If the segment is covered by `sendSegment`, clean its `prev` link
                    // to avoid memory leaks.
                    if (r < sendersCounter.value) segment.cleanPrev()
                    continue
                }
            }
        }
    ...
    internal inline fun AtomicRef<ChannelSegment<E>>.moveForward(to: ChannelSegment<E>): Boolean = loop { cur ->
        ...
        if (to.isRemoved) {
            ...
            // Restart [AtomicRef<S>.findSegmentAndMoveForward].
            return false
        }
